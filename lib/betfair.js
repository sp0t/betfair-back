...
const getBtOdds = async(sportId, leagueids, markets, inPlayOnly, username, password, sportname, monitid, tm) => {
  ...
  console.log('start inserting....', marketbook.length)

  for (var x in marketbook) {

    console.log('marketid ===========> ', marketbook[x].marketId);
    console.log('insert', playname[marketbook[x].marketId].away, playname[marketbook[x].marketId].home)
