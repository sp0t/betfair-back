...
const getBtOdds = async(sportId, leagueids, markets, inPlayOnly, username, password, sportname, monitid, tm) => {
  ...
  for (var x in marketbook) {
    ...
    funcs.push(saveData(sportId, monitid, sportname, competitionId[marketbook[x].marketId], competitionName[marketbook[x].marketId], events[marketbook[x].marketId], eventName[marketbook[x].marketId], playname[marketbook[x].marketId].away, playname[marketbook[x].marketId].home, tm, matchday[marketbook[x].marketId], market));
  }
