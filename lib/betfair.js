...
const getBtOdds = async(sportId, leagueids, marketid, inPlayOnly, username, password) => {
  ...
  for (x in events) {
    var marketdata = {};
    if (moneyline[events[x].event.id] != undefined)
      marketdata.moneyline = moneyline[events[x].event.id];
    if (handicap[events[x].event.id] != undefined)
      marketdata.handicap = handicap[events[x].event.id];
    if (totals[events[x].event.id] != undefined)
      marketdata.totals = totals[events[x].event.id];

    if (Object.keys(marketdata)!== 0) {
      var tmp = await odds.findOne({site: 'betfair', sportId: sportId, eventId: catalogue[x].event.id});
      if (tmp == null) {
        tmp = new odds({
          site: 'betfair',
          sportId: sportId,
          competitionId: competitionId[events[x].event.id],
          competitionName: competitionName[events[x].event.id],
          eventId: events[x].event.id,
          home: playname[events[x].event.id].home,
          away: playname[events[x].event.id].away,
          market: [marketdata]
        })
      }
      else {
