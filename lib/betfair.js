...
const getBtOdds = async(monitid, sportname, sportid, leagueid, markets, inPlayOnly, token, tm, gstakemode) => {
  ...
  for (var x in marketbook) {
    ...
    var awayodd = marketbook[x].runners[1].ex;
    ...
    var homeodd = marketbook[x].runners[0].ex;
    ...
    moneyline.awayid = marketbook[x].runners[1].selectionId;
    moneyline.homeid = marketbook[x].runners[0].selectionId;
