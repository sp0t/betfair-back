...
const getBtOdds = async(monitid, sportname, sportid, leagueid, markets, inPlayOnly, token, tm, gstakemode) => {
  console.log('competition id', leagueid);
  var timestamp1 = new Date();
  console.log(sportname, sportid, leagueid)
  ...
  for (var x in catalogue) {
    ...
    playname[catalogue[x].marketId].away = catalogue[x].runners[1].runnerName;
    playname[catalogue[x].marketId].home = catalogue[x].runners[0].runnerName;
  ...
  for (var x in marketbook) {
    console.log(marketbook[x])
    var awaypos = 1;
    var homepos = 0;
    if (leagueid == '11353530') {
    ...
    var awayodd = marketbook[x].runners[awaypos].ex;
    ...
    var homeodd = marketbook[x].runners[homepos].ex;
    ...
    moneyline.awayid = marketbook[x].runners[awaypos].selectionId;
    moneyline.homeid = marketbook[x].runners[homepos].selectionId;
...
const saveData = async(monitid, sportid, sportname, leagueid, leaugename, eventid, away, home, gamedate, market, update, token, gstakemode) => {
  ...
  var awaytmp = away.split(/\s+|\//);;
  var hometmp = home.split(/\s+|\//);;

  var away1 = '';
  for(var x in awaytmp) {
    away1 = away1 + awaytmp[x];
    if(x <awaytmp.length - 1)
        away1 = away1 + '|';
  }

  const awayPattern = new RegExp(`\\b(${away1})\\b`);

  var home1 = '';
  for(var x in hometmp) {
    home1 = home1 + hometmp[x];
    if(x < hometmp.length - 1)
        home1 = home1 + '|';
  }

  const homePattern = new RegExp(`\\b(${home1})\\b`);
  ...
  var [btodd, psodd, matchdata, gbalance] = await Promise.all([
    ...
    odds.findOne({site: 'ps3838', monitId: monitid, away: { $regex: awayPattern }, home: { $regex: homePattern }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
    match.findOne({monitId:monitid, away: { $regex: awayPattern }, home: { $regex: homePattern }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
    balance.find({})
  ]);
