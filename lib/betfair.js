...
const getBtOdds = async(monitid, sportname, sportid, leagueid, markets, inPlayOnly, token, tm, gstakemode) => {
  ...
  for (var x in marketbook) {
    ...
    console.log('marketbook[x].runners[1].ex', marketbook[x].runners[1].ex)
    ...
    console.log(' marketbook[x].runners[0].ex;',  marketbook[x].runners[0].ex)
...
const saveData = async(monitid, sportid, sportname, leagueid, leaugename, eventid, away, home, gamedate, market, update, token, gstakemode) => {
  ...
  var gameregex = gamedate.slice(0, 16);
  var [btodd, psodd, matchdata, gbalance] = await Promise.all([
    ...
    odds.findOne({site: 'ps3838', monitId: monitid, away: {$regex: away , $options: 'i' }, home: {$regex: home , $options: 'i' }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
    match.findOne({monitId:monitid, away: {$regex: away , $options: 'i' }, home: {$regex: home , $options: 'i' }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
    balance.find({})
  ]);
  ...
  if (btodd == null) {
    ...
    // console.log(btodd.market[btodd.market.length - 1].moneyline)
    // console.log(odddata)
