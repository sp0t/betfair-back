...
const placebet = async(site1, competition1, site2, competition2, betrate, seesion) => {
    ...
    for (var x in odd1) {
        if (1) {
            for (var y in odd2) {
                console.log('betfair...===>', odd1[x].away, odd1[x].home);
                console.log('ps3838...===>', odd2[y].away, odd2[y].home);
                if ((odd1[x].home == odd2[y].home) && (odd1[x].away == odd2[y].away)) {

                    console.log('teams...===>', odd1[x].away, odd1[x].home);
                    ...
                    for (var z in betrate) {
                        if (btawayprice < bthomeprice) {
                            if ((betrate[z].from <= Math.abs(btawayprice - psawayprice)) && Math.abs((btawayprice - psawayprice) < betrate[z].to)) 
                            ...
                            if ((betrate[z].from <= Math.abs(bthomeprice - pshomeprice)) && Math.abs((bthomeprice - pshomeprice)<= betrate[z].to))
