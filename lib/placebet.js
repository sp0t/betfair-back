...
const placebet = async(site1, competition1, site2, competition2, betrate) => {
    ...
    for (var x in odd1) {
        if ((current - odd1[x].update) < 30000000) {
            for (var y in odd2) {
                if ((odd1[x].home == odd2[y].home) && (odd1[x].away == odd2[y].away)) {
                    ...
                    for (var z in betrate) {
                        if(btawayprice <= psawayprice) {
                            if (betrate[z].from <= (btawayprice - psawayprice) &&  (btawayprice - psawayprice)<= betrate[z].to)
                                funcs.push(betting(odd1val.moneyline.marketid, odd1val.moneyline.awayid, btawayprice, betrate[z].price, token))
                        }
                        else {
                            if (betrate[z].from <= (bthomeprice - pshomeprice) &&  (bthomeprice - pshomeprice)<= betrate[z].to)
                                funcs.push(betting(odd1val.moneyline.marketid, odd1val.moneyline.homeid, bthomeprice, betrate[z].price, token))
                        }
                    }
                }
            }
...
const betting = async(marketid, selectionId, price, size, token) => {
    var options = {
        headers: {
          'X-Application': 'XCy3BR7EjehV32o3',
          'Accept':'application/json'
