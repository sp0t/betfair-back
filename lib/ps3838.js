...
const getPsOdds = async(monitid, sportname, sportId, leagueids, lineId, isLive, token, tm) => {
  ...
  for (var x in eventsFromOdd) {
    ...
    if(market.hasOwnProperty('moneyline'))
      funcs.push(saveData(sportId, monitid, sportname, eventsFromOdd[x].leagueId,  eventsFromOdd[x].leagueName, x, '0', eventsFromOdd[x].away, eventsFromOdd[x].home, tm, eventsFromOdd[x].matchday, market));
    }
