...
const saveData = async(monitid, sportid, sportname, leagueid, leaugename, eventid, away, home, gamedate, market, update, gstakemode) => {
  ...
  var [psodd, btodd, matchdata] = await Promise.all([
    odds.findOne({site: 'ps3838', monitId: monitid, $or:[{away: away}, {home: home}], gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
    odds.findOne({site: 'betfair', monitId: monitid, $or:[{away: away}, {home: home}], gamedate: { $regex: gameregex, $options: 'i' }, state: 0, update: update}),
