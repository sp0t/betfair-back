...
const getPsOdds = async(sportId, leagueids, lineId, isLive, token) => {
  ...
  for (var x in eventsFromOdd) {
    ...
    if (eventsFromOdd[x].spreads) market.spreads = eventsFromOdd[x].spreads;
    if (eventsFromOdd[x].spreads) market.moneyline = eventsFromOdd[x].moneyline;
    if (eventsFromOdd[x].spreads) market.totals = eventsFromOdd[x].totals;
    if (eventsFromOdd[x].teamTotal) market.teamTotal = eventsFromOdd[x].teamTotal;
    if (tmp == null) {
      tmp = new odds({
        site: 'ps3838',
        sportId: sportId,
