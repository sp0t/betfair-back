...
const getPsOdds = async(monitid, sportname, sportid, leagueid, lineId, isLive, token, tm, gstakemode) => {
  ...
  console.log('ps3838========================>', sportid, leagueid, lineId)
    ...
    for (var x in oddData) {
      ...
      if (odd != undefined) {
        ...
        if (fixture != undefined) {
          ...
          console.log('odd.moneyline', odd.moneyline)
          ...
          if (market.moneyline.away != '-' || market.moneyline.home != '-') {
            console.log('insert data=============>', sportname, market)
...
const saveData = async(monitid, sportid, sportname, leagueid, leaugename, eventid, away, home, gamedate, market, update, gstakemode) => {
  ...
  var awaytmp = away.split(' ');
  var hometmp = home.split(' ');
  ...
  console.log('========================',away1, home1 )
  console.log(away, home, gameregex, update);
  ...
  var [psodd, btodd, matchdata] = await Promise.all([
    ...
    odds.findOne({site: 'betfair', monitId: monitid, away: { $regex: awayPattern }, home: { $regex: homePattern }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0, update: update}),
