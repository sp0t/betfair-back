...
const sendMatchData = async() => {
  ...
  const sendOddData = async() => {
    ...
    var awaytmp = ret.away.split(/\s+|\//);;
  var hometmp = ret.home.split(/\s+|\//);;
  ...
  const homePattern = new RegExp(`\\b(${home1})\\b`);
    ...
    try{
      var [psodd, btodd] = await Promise.all([
         odds.find({site:'ps3838', monitId:ret.monitid, away: { $regex: awayPattern }, home: { $regex: homePattern }, home:{$regex: ret.home , $options: 'i' }, state: 0}),
