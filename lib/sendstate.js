  ...
  const sendOddData = async() => {
    ...
    try{
      var [psodd, btodd] = await Promise.all([
         odds.find({site:'ps3838', monitId:ret.monitid, away:ret.away, home:ret.home, state: 0}),
        odds.findOne({site: 'ps3838', monitId: monitid, away: {$regex: away , $options: 'i' }, home: {$regex: home , $options: 'i' }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
         odds.find({site:'betfair', monitId:ret.monitid, away: {$regex: ret.away , $options: 'i' }, home: {$regex: ret.home , $options: 'i' }, state: 0})
        ])
      var result = {};
