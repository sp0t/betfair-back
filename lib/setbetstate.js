...
const setBetState = async(token) => {
    ...
    for (x in data) {
        ...
        console.log(current, update, 'difference=======>', current - update)
...
const uploadToS3Buckt = async(data, token) => {
    var s3data = {};
    var gameregex = data.gamedate.slice(0, 10);

    var [psodd, btodd, gbalance] = await Promise.all([
        odds.find({site:'ps3838', monitId:data.monitId, away:data.away, home:data.home, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
        odds.find({site:'betfair', monitId:data.monitId, away:data.away, home:data.home, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
        balance.find({})
        ])

    console.log(psodd, btodd)

    if (psodd.length > 0 && btodd.length > 0) {
        console.log('===============start')
        psodd[0].state = 1;
        btodd[0].state = 1;
