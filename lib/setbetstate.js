...
const uploadToS3Buckt = async(data, token) => {
    ...
    var [psodd, btodd, gbalance] = await Promise.all([
        odds.find({site:'ps3838', monitId:data.monitId, away: {$regex: data.away , $options: 'i' }, home: {$regex: data.home , $options: 'i' }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
