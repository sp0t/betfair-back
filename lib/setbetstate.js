...
const uploadToS3Buckt = async(data, token) => {
    ...
    var [psodd, btodd, gbalance] = await Promise.all([
        odds.find({site:'ps3838', monitId:data.monitId, away: {$regex: away , $options: 'i' }, home: {$regex: home , $options: 'i' }, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
        odds.find({site:'betfair', monitId:data.monitId, away: away, home: home, gamedate: { $regex: gameregex, $options: 'i' }, state: 0}),
        balance.find({})
        ])
