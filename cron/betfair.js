        ...
        for (var x in monitor) {
          for (var y in monitor[x].sites) {
            if (monitor[x].sites[y].name == 'betfair') {
              funcs.push(getBtOdds(monitor[x].mornitId, monitor[x].sport, monitor[x].sites[y].sportid, monitor[x].sites[y].competition, ['Moneyline', 'Total Points', 'Handicap'], monitor[x].playmode, session[0].username, session[0].password, updatetm))
            }

            if (monitor[x].sites[y].name == 'ps3838') {
              funcs.push(getPsOdds(monitor[x].mornitId, monitor[x].sport, monitor[x].sites[y].sportid, monitor[x].sites[y].competition, 0, monitor[x].playmode, token, updatetm))
            }
          }
        }
